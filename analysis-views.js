(function(){
  ...
  function drawGeometry(){
    const canvas=$('geometryCanvas');if(!canvas)return;const {ctx,w,h}=fit(canvas),g=window.cfdApp.getGeometry();ctx.clearRect(0,0,w,h);
    ...
    ctx.beginPath();for(let i=0;i<=240;i++){const x=i/240,y=window.cfdApp.sectionY(x).upper;i?ctx.lineTo(X(x),Y(y)):ctx.moveTo(X(x),Y(y))}for(let i=240;i>=0;i--){const x=i/240;ctx.lineTo(X(x),Y(window.cfdApp.sectionY(x).lower))}ctx.closePath();ctx.fillStyle='rgba(80,216,244,.10)';ctx.fill();ctx.strokeStyle='#50d8f4';ctx.lineWidth=1.6;ctx.stroke();
    ctx.beginPath();for(let i=0;i<=240;i++){const x=i/240,s=window.cfdApp.sectionY(x),y=.5*(s.upper+s.lower);i?ctx.lineTo(X(x),Y(y)):ctx.moveTo(X(x),Y(y))}ctx.strokeStyle='#ffad72';ctx.lineWidth=1.1;ctx.setLineDash([4,3]);ctx.stroke();ctx.setLineDash([]);

    let maxT={v:0,x:0,u:0,l:0},maxC={v:-Infinity,x:0};let area=0,prev=null;
    for(let i=0;i<=500;i++){const x=i/500,s=window.cfdApp.sectionY(x),t=s.upper-s.lower,c=.5*(s.upper+s.lower);if(t>maxT.v)maxT={v:t,x,u:s.upper,l:s.lower};if(c>maxC.v)maxC={v:c,x};if(prev)area+=.5*(t+prev.t)*(x-prev.x);prev={x,t}}
    ...
    leLine(x=>CFDSectionY(x,CFDDefaultGeometry),'rgba(184,202,214,.48)',[4,3]);leLine(x=>window.cfdApp.sectionY(x),'#50d8f4');
  ...
  function initializeViews(){window.addEventListener('cfdgeometrychange',()=>{drawGeometry()});window.addEventListener('cfdhistory',queueDraw);window.addEventListener('resize',queueDraw);drawGeometry();drawConvergence()}
